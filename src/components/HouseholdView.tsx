          ...
          <button
            onClick={() => {
              ...
              fetch("/api/logs")
                .then(r => r.json())
                .then(data => {
                  const filtered = (Array.isArray(data) ? data : []).filter((log: any) => 
                    (log.action || "").toLowerCase().includes("hộ gia đình") ||
                    (log.action || "").toLowerCase().includes("nhân khẩu") ||
                    (log.details || "").toLowerCase().includes("hộ")
                  );
            ...
            className="flex items-center gap-1.5 bg-slate-100 hover:bg-slate-200 text-slate-800 px-3.5 py-2 rounded-xl text-xs font-semibold border border-slate-300 transition-colors cursor-pointer shadow-xs"
            title="Xem lịch sử chỉnh sửa và lưu vết thao tác người dùng"
          >
            <HistoryIcon className="w-3.5 h-3.5 text-slate-600" />
            Lịch sử cập nhật
          </button>
          ...
          <div className="flex items-center gap-2">
            ...
            {onSync && (
              <button
                onClick={onSync}
                disabled={isSyncing}
                className="flex items-center gap-1.5 text-xs font-bold text-emerald-700 hover:text-emerald-800 bg-emerald-50 hover:bg-emerald-100 px-3 py-1.5 rounded-xl border border-emerald-200 transition-colors cursor-pointer disabled:opacity-50 shadow-2xs"
                title="Đồng bộ lại toàn bộ dữ liệu máy chủ"
              >
                <RefreshCw className={`w-3.5 h-3.5 ${isSyncing ? "animate-spin" : ""}`} />
                Đồng bộ lại tất cả dữ liệu
              </button>
      ...
      <div className="bg-gradient-to-r from-emerald-500/10 to-teal-500/10 border border-emerald-500/20 rounded-2xl p-4 md:p-5 shadow-xs space-y-3.5">
        ...
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-11 pr-4 py-3 bg-white border-2 border-emerald-200/60 rounded-xl text-sm text-slate-800 placeholder-slate-400 focus:outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200"
          />
        </div>
                  ...
                  <div className="flex-1 space-y-2 text-xs text-slate-700">
                    ...
                    <p className="flex items-center gap-1">
                      ...
                      <span className="font-mono font-medium text-slate-800">
                        {selectedHousehold.gpsLat ?? 11.34372}, {selectedHousehold.gpsLng ?? 106.122664}
                      </span>
                    </p>
                    ...
                    <>
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-slate-200/80 pb-2">
                        ...
                        {hhMembers.length > 0 && (
                          <button
                            type="button"
                            onClick={() => toggleAllMemberHistory(hhMembers.map(m => m.id))}
                            className="flex items-center gap-1.5 text-xs font-bold text-amber-900 bg-amber-100/80 hover:bg-amber-200/80 px-3 py-1.5 rounded-xl border border-amber-300/90 transition cursor-pointer shadow-2xs self-start sm:self-auto"
                            title="Ẩn hoặc hiện lịch sử cập nhật cho tất cả thành viên trong hộ gia đình"
                          >
                            <HistoryIcon className="w-3.5 h-3.5 text-amber-700" />
                            {hhMembers.every(m => expandedMemberHistory[m.id]) ? "Ẩn tất cả lịch sử" : "Hiện tất cả lịch sử cập nhật"}
                          </button>
                        )}
                      </div>
                          ...
                          {hhMembers.map((m) => {
                            ...
                            return (
                              ...
                              >
                                ...
                                {/* Action bar: History Button & Zoom Photo */}
                                <div className="mt-2 pt-2 border-t border-slate-100 flex flex-wrap items-center justify-between gap-2">
                                  <button
                                    type="button"
                                    onClick={() => toggleMemberHistory(m.id)}
                                    className={`flex items-center gap-1.5 text-xs font-bold px-3 py-1.5 rounded-xl border transition cursor-pointer shadow-2xs ${
                                      isHistoryExpanded
                                        ? "bg-amber-600 text-white border-amber-700 hover:bg-amber-700"
                                        : "bg-amber-50 hover:bg-amber-100 text-amber-900 border-amber-300/80"
                                    }`}
                                    title="Nhấn để ẩn/hiện chi tiết lịch sử cập nhật và nhật ký biến động của nhân khẩu này"
                                  >
                                    <HistoryIcon className="w-3.5 h-3.5" />
                                    {isHistoryExpanded ? "Ẩn lịch sử cập nhật" : "Hiện lịch sử cập nhật"}
                                    <span className={`text-[10px] px-1.5 py-0.2 rounded-full font-bold ml-0.5 ${isHistoryExpanded ? "bg-amber-800 text-amber-100" : "bg-amber-200/90 text-amber-950"}`}>
                                      {memberLogs.length}
                                    </span>
                                  </button>
                                ...
                                {/* Detailed History Timeline Box */}
                                {isHistoryExpanded && (
                                  <div className="mt-2.5 bg-amber-50/90 border border-amber-300 rounded-xl p-3.5 space-y-2.5 text-xs text-amber-950 animate-fade-in shadow-2xs">
                                    <div className="flex items-center justify-between border-b border-amber-200/80 pb-2">
                                      <span className="font-bold text-amber-900 flex items-center gap-1.5">
                                        <Clock className="w-4 h-4 text-amber-700" />
                                        Nhật ký lưu vết & Lịch sử biến động: <span className="underline">{m.fullName}</span>
                                      </span>
                                      <span className="text-[10px] font-bold text-amber-950 bg-amber-200/80 px-2 py-0.5 rounded-md border border-amber-300">
                                        {memberLogs.length} bản ghi
                                      </span>
                                    </div>
                                        ...
                                        <div key={log.id} className="bg-white p-2.5 rounded-lg border border-amber-200/90 shadow-2xs space-y-1">
                                          <div className="flex items-center justify-between text-[11px] border-b border-slate-100 pb-1">
                                            <span className="font-bold text-slate-800 flex items-center gap-1">
                                              <UserIcon className="w-3 h-3 text-amber-700 shrink-0" />
                                              {log.userName}
                  ...
                  <div>
                    ...
                    <div className="flex gap-2">
                      ...
                      <button
                        type="button"
                        onClick={() => setIsQrModalOpen(true)}
                        className="bg-emerald-50 hover:bg-emerald-100 text-emerald-800 px-2.5 py-2 rounded-xl text-xs font-bold border border-emerald-300 transition-colors shrink-0 flex items-center gap-1 cursor-pointer"
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 pt-2 border-t border-slate-200/80">
                  <div>
                    ...
                    >
                      ...
                      <option value={WasteCollectionStatus.CANCELLED}>Đã hủy dịch vụ</option>
                    </select>
                  </div>
